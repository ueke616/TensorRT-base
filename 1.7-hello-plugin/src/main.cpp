...
using namespace std;
/*
在C++中，`inline`修饰符对于函数有两个主要作用：

1. **建议编译器内联展开**：`inline`关键字首先是一个向编译器提出的建议，它建议编译器在每个调用点
将函数的代码直接展开，而不是通过常规的函数调用机制执行。这意味着函数的每次调用都被替换为函数体的一个副本。这样做的好处是可以减少函数调用的开销（如参数传递、栈帧的创建和销毁等），对于小型且频繁调用的函数，这可以提高程序的执行效率。然而，这只是一个建议，编译器可以选择忽略它，特别是对于复杂或大型的函数。

2. **允许在头文件中定义函数**：在C++中，如果在多个源文件中包含了同一个头文件，而这个头文件中定义了
一个非`inline`函数，那么在链接时会因为多次定义同一个函数而导致链接错误（多重定义错误）。使用`inline`修饰符可以避免这种问题，因为它允许在头文件中定义函数，同时告诉链接器忽略多重定义的问题，只要所有的定义都是相同的。这使得`inline`函数成为在头文件中定义小型模板函数或包含器函数的理想选择。

在你的例子中，`inline`修饰符的使用意味着这个`severity_string`函数可以在头文件中定义，并在多个源文件中
包含而不会导致链接错误。此外，它也建议编译器在每个调用点直接展开这个函数，这对于这样的简单转换函数来说，是有益的，因为它可以减少函数调用的开销。
*/
...
bool build_model() {
    ...
    // 通过onnxparser解析器解析的结果会填充到network中, 类似 addConv 的方式添加进去
    ...
    config->setMaxWorkspaceSize(1 << 28);  // 分配GPU空间最大256MB
